...
export interface ClaimSlice {
  ...
  createClaim: (data: Partial<Claim>) => Promise<{ success: boolean; claim?: Claim }>;
  updateClaim: (id: string, data: Partial<Claim>) => Promise<{ success: boolean; claim?: Claim }>;
...
export const createClaimSlice: StateCreator<ClaimSlice> = (set, get) => ({
  ...
  fetchClaimsByUser: async (userId) => {
    try {
      ...
      const claims = result.data?.data || result;
  ...
  fetchClaimById: async (id) => {
    try {
      ...
      const result = await res.json();
      const claim = result.data?.data || result;
  ...
  createClaim: async (data) => {
    try {
      ...
      const res = await fetch(`${API_CONFIG.baseUrl}/claims`, {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
      ...
      if (!res.ok) throw new Error(`Failed to create claim: ${res.statusText}`);
  ...
  updateClaim: async (id, data) => {
    try {
      ...
      if (!res.ok) throw new Error(`Failed to update claim: ${res.statusText}`);

      const result = await res.json();
      const updatedClaim = result.data?.data || result;

      set((state) => ({
        claims: state.claims.map((c) => (c.id === id ? updatedClaim : c)),
        selectedClaim:
          state.selectedClaim && state.selectedClaim.id === id
            ? updatedClaim
            : state.selectedClaim,
        isLoadingClaim: false,
      }));
